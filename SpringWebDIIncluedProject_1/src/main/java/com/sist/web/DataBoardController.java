...
public class DataBoardController {
	...
	//databoard/detail.do?no=${vo.no } => 요청 데이터가 있는 경우
	// 요청 데이터가 없는 경우 => default값을 설정해야되기 때문에 String으로 설정
	// 모든 데이터는 String으로 받을 수 있다.
	// 매개변수는 순서가 없다, 여러개를 받을 수 있다.
	/*
	   MVC의 핵심 => 매개변수/ 리턴형
	   					  ----- 화면이동 (forward/ sendRedirect)
	   					  		forward => 화면 변경 => request값을 보냄
	   					  			return "main/main"
	   					  		sendredirect => 화면 변경=> request값 초기화 => 기존에 설정된 위치로 변경
	   					 			return "redirect:.do"
	   1) Model(Controller) => 매개변수 설정
	      매개변수 : 사용자가 보내준 값을 받는다 (
		          ...
		          내장 객체
		          ------
		          HttpServletRequest, HttpServleResponse
		          --------------------------------------
		          	=> 다운로드 , Cookie 사용
		...
		Manager: Open API
		RestController : JavaScript로 JSON 전송할 목적
		-------------- 다른 언어와 연결=> 데이터를 전송
		          	   ------
		          	   1. WEB => JavaScript(Ajax,Vuejs,React)
		          	   2. Mobile => Kotlin
		          	
		Service : BI => DAO여러개 통합 , 다른 서비스 통합 (크롤링)          	   
	 */
