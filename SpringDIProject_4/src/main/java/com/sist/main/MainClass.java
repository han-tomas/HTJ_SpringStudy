...
/*
	이미 제작되어 있는 라이브러리
	--------------------
	1. Container : 사용자가 등록한 클래스를 관리 (POJO: 일반 클래스, 상속이 없는 상태의 클래스)
			...
			=> 동작
				XML을 제공 => Container
							...
							2. ApplicationContext (XML) => core + AOP
								=> 순수하게 자바로만 이루어짐 (보안) => Spring 5(보안을 강조)
				...
				= 의존성 주입 (DI)
					...
					<bean id="" class=""
						...
						개발자가 등록된 클래스 객체를 활용 (스프링에서 처리되는 부분이 아니다)
						destroy-method : 객체 소멸시 호출
						--------------- db.close
							소멸
						
						*** 일반 멤버변수는 데이터베이스 정보외에는 거의 사용빈도가 없다.
					/>	
	   	=> 스프링을 통해서 프로그램 동작을 위한, 필요하 데이터(중복 제거)
	   			=> 결합성을 낮게 만든다
	   			   ---------
	   			    클래스에 영향(유집수에 최적화된 프레임워크)
		   클래스에 설정된 변수에 대한 값을 주입
		   ---------------------- 초기화
		   1. 명시적 초기화 (X)
		   2. 생성자 ========> 매개변수에 값을 지정
		   3. setter ======> setXxx()애 값을 지정
		   	  property
		   	  
		===> 클래스가 많다면 XML이 커진다(속도가 느려진다)
			 ---------------------
			 			|
			 		패키지단위로 등록
			 		-------
