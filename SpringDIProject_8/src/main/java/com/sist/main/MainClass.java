		...
		= 프레임워크 : 레고
			...
			=> 스프링의 목표(로드 존스) => Open Source
				1) 클래스와 클래스간의 느슨한 결합, 낮은 결합도 유지
								   -----------------------
								   	변경,추가할때 다른 클래스에 영향이 없이 개발
								   	POJO(독립적인 클래스 제작)
				2) 메소드마다 반복해서 사용하는 소스코드 => 공통 모듈로 분리(응집력이 높은 프로그램 제작)				   	
					=> AOP : OOP를 보완
			=> 클래스를 모아서 관리(생성 to 소멸) => 컨테이너
				...
				= 클래스명(메모리 할당이 가능)을 전송
				  ...
				  Annotation
				  	=> 메모리 할당
				  		...
				  		@Controller : MVC 구조의 Model(페이지 지정)
				  		@RextController :  MVC 구조의 Model => JSON 전송
